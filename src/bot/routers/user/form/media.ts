...
const formProceedKb = new InlineKeyboard().text("✅ Да", "proceed_form").text("❌ Отмена", "cancel_form");
...
router.on(":media", async (ctx, next) => {
  if (ctx.session.state != State.media || !ctx.message || !ctx.session.formData.location) return next();
